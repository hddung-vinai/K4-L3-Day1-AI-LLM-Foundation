...
def format_comparison_table(results: list[dict]) -> str:
    ...
    # Header
    header = (
        f"{'Prompt':<42} | {'GPT-4o Response':<43} | "
        f"{'Mini Response':<43} | {'GPT-4o Lat.':<12} | {'Mini Lat.':<10}"
    )
    ...
    for r in results:
        prompt = truncate(r.get("prompt", ""), 40)
        gpt4o_resp = truncate(r.get("gpt4o_response", ""), 40)
        mini_resp = truncate(r.get("mini_response", ""), 40)
        gpt4o_lat = f"{r.get('gpt4o_latency', 0):.2f}s"
        mini_lat = f"{r.get('mini_latency', 0):.2f}s"

        row = (
            f"{prompt:<42} | {gpt4o_resp:<43} | "
            f"{mini_resp:<43} | {gpt4o_lat:<12} | {mini_lat:<10}"
        )
        rows.append(row)
